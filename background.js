...
class Particle {
    ...
    draw() {
        ...
        ctx.fillStyle = 'rgba(148, 163, 184, 0.1)'; // Very subtle slate color
        ctx.fill();
    }
    ...
    update() {
        ...
        // Smoother interaction: gently push away
        if (distance < mouse.radius + this.size) {
            if (mouse.x < this.x && this.x < canvas.width - this.size * 10) {
                this.x += 2; // Reduced speed
            }
...
function init() {
    particlesArray = [];
    // Reduced density: increased divisor from 9000 to 15000
...
window.addEventListener('resize',
    function() {
        // Recalculate based on container size, not window
        canvas.width = canvas.parentElement.offsetWidth;
        canvas.height = canvas.parentElement.offsetHeight;
